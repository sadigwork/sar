...
const mockApplications = [
  {
    ...
    notes: "Your application is currently being reviewed by our technical committee.",
    missingDocuments: [],
    timeline: [
      {
  ...
  },
  {
    ...
    notes: "Please provide the missing documents to proceed with your application.",
    missingDocuments: ["Experience certificate", "Professional development proof"],
    timeline: [
      {
        date: "2023-10-05T09:15:00Z",
        status: "submitted",
        title: "Application Submitted",
        description: "Your renewal application has been successfully submitted.",
      },
      {
      ...
      },
      {
        ...
        description: "Additional documents are required to proceed with your application.",
      },
    ],
    documents: [
      {
      ...
      },
      {
        ...
        rejectionReason: "The document is outdated. Please provide recent professional development activities.",
      },
    ],
    messages: [
      {
        ...
        date: "2023-10-05T09:15:00Z",
        from: "system",
        content: "Your renewal application has been received and is now under review.",
      },
      {
        id: "msg2",
        date: "2023-10-25T11:20:00Z",
...
export default function ApplicationDetailPage({ params }) {
  ...
  // Check if user is logged in and fetch application data
  useEffect(() => {
    if (!user) {
      router.push("/login")
      return
    ...
    setTimeout(() => {
      ...
      if (app) {
        ...
        toast({
          title: t("language") === "en" ? "Application not found" : "لم يتم العثور على الطلب",
          description:
            t("language") === "en"
      ...
      case "pending":
        return (
          <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200">
            <Clock className="mr-1 h-3 w-3" /> {t("language") === "en" ? "Under Review" : "قيد المراجعة"}
          </Badge>
        )
      case "approved":
        return (
          <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
            <CheckCircle className="mr-1 h-3 w-3" /> {t("language") === "en" ? "Approved" : "تمت الموافقة"}
          </Badge>
        )
      case "rejected":
        return (
          <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">
            <XCircle className="mr-1 h-3 w-3" /> {t("language") === "en" ? "Rejected" : "مرفوض"}
          </Badge>
        )
      case "action_required":
        return (
          <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">
            <AlertCircle className="mr-1 h-3 w-3" /> {t("language") === "en" ? "Action Required" : "إجراء مطلوب"}
          </Badge>
        )
      default:
  ...
  const getDocumentStatusBadge = (status) => {
    switch (status) {
      case "verified":
        return (
          <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
            <CheckCircle className="mr-1 h-3 w-3" /> {t("language") === "en" ? "Verified" : "تم التحقق"}
          </Badge>
        )
      case "pending":
        return (
          <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200">
            <Clock className="mr-1 h-3 w-3" /> {t("language") === "en" ? "Pending" : "قيد الانتظار"}
          </Badge>
        )
      case "rejected":
        return (
  ...
  const handleFileUpload = (file) => {
    toast({
      ...
      description:
        t("language") === "en" ? `${file.name} has been uploaded successfully.` : `تم رفع ${file.name} بنجاح.`,
    })
  }
  ...
  const handleSendMessage = () => {
    ...
    toast({
      ...
      description:
        t("language") === "en" ? "Your message has been sent to the reviewer." : "تم إرسال رسالتك إلى المراجع.",
    })
    ...
    return (
      <div className="container py-10">
        <Alert variant="destructive">
          <AlertCircle className="h-4 w-4" />
          <AlertTitle>{t("language") === "en" ? "Application not found" : "لم يتم العثور على الطلب"}</AlertTitle>
          <AlertDescription>
            {t("language") === "en"
              ? "The application you're looking for doesn't exist or you don't have permission to view it."
              : "الطلب الذي تبحث عنه غير موجود أو ليس لديك إذن لعرضه."}
        ...
        <div className="flex justify-center mt-6">
          <Button onClick={() => router.push("/dashboard")}>
            ...
            {t("language") === "en" ? "Back to Dashboard" : "العودة إلى لوحة التحكم"}
          </Button>
        </div>
      </div>
    )
  ...
  return (
    <div className="container py-10">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold">{t("language") === "en" ? "Application Details" : "تفاصيل الطلب"}</h1>
      </div>
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h2 className="text-xl font-semibold mb-1">
                {t("language") === "en" ? application.type : getArabicType(application.type)} - {application.level}
              </h2>
              <p className="text-muted-foreground">
                {application.specialization} | {t("language") === "en" ? "Submitted on" : "تم التقديم في"}:{" "}
            ...
            <div className="flex flex-col items-end">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {t("language") === "en" ? "Last updated" : "آخر تحديث"}: {formatDate(application.lastUpdated)}
              </p>
            </div>
          </div>
          ...
          <div className="mt-6">
            <div className="flex justify-between mb-1">
              <span className="text-sm text-muted-foreground">{t("language") === "en" ? "Progress" : "التقدم"}</span>
              <span className="text-sm font-medium">{application.progress}%</span>
            </div>
            <div className="w-full bg-secondary rounded-full h-2.5">
              <div className="bg-primary h-2.5 rounded-full" style={{ width: `${application.progress}%` }}></div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            ...
            {application.estimatedCompletion && (
              <div>
                <p className="text-sm text-muted-foreground mb-1">
                  {t("language") === "en" ? "Estimated Completion" : "الإنجاز المتوقع"}
                </p>
          ...
          {application.status === "action_required" && application.missingDocuments.length > 0 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2">
          <Tabs defaultValue="timeline">
            <TabsList className="grid grid-cols-3 mb-6">
              <TabsTrigger value="timeline">{t("language") === "en" ? "Timeline" : "الجدول الزمني"}</TabsTrigger>
              <TabsTrigger value="documents">{t("language") === "en" ? "Documents" : "المستندات"}</TabsTrigger>
              <TabsTrigger value="messages">{t("language") === "en" ? "Messages" : "الرسائل"}</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="timeline">
              <Card>
                <CardHeader>
                  <CardTitle>{t("language") === "en" ? "Application Timeline" : "الجدول الزمني للطلب"}</CardTitle>
                  <CardDescription>
                    {t("language") === "en" ? "Track the progress of your application" : "تتبع تقدم طلبك"}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    <div className="space-y-8">
                      ...
                      {application.nextStep && (
                        <div className="relative flex gap-4">
                          ...
                          <div className="flex-1 pt-1">
                            <div className="flex items-center justify-between">
                              <h4 className="font-medium text-muted-foreground">{application.nextStep}</h4>
            ...
            <TabsContent value="documents">
              <Card>
                <CardHeader>
                  <CardTitle>{t("language") === "en" ? "Application Documents" : "مستندات الطلب"}</CardTitle>
                  <CardDescription>
                    {t("language") === "en"
                ...
                <CardContent>
                  <div className="space-y-6">
                    {application.documents.map((doc) => (
                      <div key={doc.id} className="flex items-start justify-between border-b pb-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              {t("language") === "en" ? "Uploaded on" : "تم الرفع في"}: {formatDate(doc.uploadDate)}
                            </p>
                            {doc.status === "rejected" && doc.rejectionReason && (
                              <p className="text-sm text-red-600 mt-1">
                                {t("language") === "en" ? "Reason" : "السبب"}: {doc.rejectionReason}
                              </p>
                            )}
                          </div>
                        </div>
                        <div className="flex flex-col items-end gap-2">
                          ...
                          {doc.status === "rejected" && (
                            <Button size="sm">
                              ...
                              {t("language") === "en" ? "Re-upload" : "إعادة الرفع"}
                            </Button>
                          )}
                        </div>
                      </div>
                    ...
                    {application.status === "action_required" && application.missingDocuments.length > 0 && (
                      <div className="mt-6 pt-4 border-t">
                        <h4 className="font-medium mb-4">
                          {t("language") === "en" ? "Upload Required Documents" : "رفع المستندات المطلوبة"}
                        </h4>
                        <FileUpload
            ...
            <TabsContent value="messages">
              <Card>
                <CardHeader>
                  <CardTitle>{t("language") === "en" ? "Messages" : "الرسائل"}</CardTitle>
                  <CardDescription>
                    {t("language") === "en" ? "Communication regarding your application" : "التواصل بخصوص طلبك"}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                      ...
                      >
                        ...
                        >
                          <div className="flex justify-between items-start mb-1">
                            ...
                            <span className="text-xs opacity-70 ml-2">{formatDateTime(message.date)}</span>
                          </div>
                          <p
                        ...
                        {message.from !== "reviewer" && message.from !== "system" && (
                  ...
                  <div className="flex gap-2 mt-4">
                    <textarea
                      ...
                      placeholder={t("language") === "en" ? "Type your message here..." : "اكتب رسالتك هنا..."}
                      value={messageText}
                      onChange={(e) => setMessageText(e.target.value)}
                    />
                    <Button className="self-end" onClick={handleSendMessage} disabled={!messageText.trim()}>
        ...
        <div>
          <Card className="mb-6">
            <CardHeader>
              <CardTitle>{t("language") === "en" ? "Application Details" : "تفاصيل الطلب"}</CardTitle>
            </CardHeader>
            <CardContent>
              <dl className="space-y-4">
                ...
                <div>
                  <dt className="text-sm text-muted-foreground">{t("language") === "en" ? "Type" : "النوع"}</dt>
                  <dd className="font-medium">
                    {t("language") === "en" ? application.type : getArabicType(application.type)}
                  </dd>
                </div>
                <div>
                  <dt className="text-sm text-muted-foreground">{t("language") === "en" ? "Level" : "المستوى"}</dt>
                ...
                <div>
                  <dt className="text-sm text-muted-foreground">
                    {t("language") === "en" ? "Submission Date" : "تاريخ التقديم"}
                  </dt>
                  <dd className="font-medium">{formatDate(application.submissionDate)}</dd>
                </div>
                <Separator />
                <div>
                  <dt className="text-sm text-muted-foreground">{t("language") === "en" ? "Reviewer" : "المراجع"}</dt>
                  <dd className="font-medium">{application.reviewer}</dd>
                </div>
                <div>
                  <dt className="text-sm text-muted-foreground">
                    {t("language") === "en" ? "Reviewer Email" : "البريد الإلكتروني للمراجع"}
                  </dt>
          ...
          <Card>
            <CardHeader>
              <CardTitle>{t("language") === "en" ? "Need Help?" : "بحاجة إلى مساعدة؟"}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Button variant="outline" className="w-full justify-start">
                  ...
                  {t("language") === "en" ? "Contact Support" : "الاتصال بالدعم"}
                </Button>
                <Button variant="outline" className="w-full justify-start">
